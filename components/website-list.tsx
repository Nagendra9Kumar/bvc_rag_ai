...
export function WebsiteList() {
  ...
  const fetchWebsites = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to load websites",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const triggerScrape = async (_id: string) => {
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to initiate scraping",
        variant: "destructive",
      });
    }
  };
  ...
  if (!loading && websites.length === 0) {
    return <div className="text-center py-6">No websites added yet. Add your first website above.</div>;
  }
            ...
            <TableRow key={website._id || `website-${index}`}>
              ...
              <TableCell className="text-right space-x-2">
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={() => triggerScrape(website._id)}
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will delete the website and all its scraped data. This action cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
